...
const NavItem = ({ icon, label, to, disabled = false, comingSoon = false }) => {
    ...
    const iconClasses = `material-symbols-outlined ${
        isActive ? 'text-[#1E3A8A]' : disabled || comingSoon ? 'text-[#9CA3AF]' : 'text-[#6B7280] group-hover:text-[#2563EB]'
    }`;
    ...
    const content = (
        <>
            ...
            <span>{label}</span>
            {comingSoon && (
                <span className="ml-auto text-xs text-[#9CA3AF] italic">(Coming Soon)</span>
            )}
        </>
    );
